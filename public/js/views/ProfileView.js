...
export default {
  ...
  setup(props, { emit }) {
    ...
    function openEditProject(proj, idx) {
    ...
    const displayName = computed(() => {
      if (props.authUser?.role === 'student') {
        return props.student?.name || props.authUser?.name || 'Student';
      }
      return props.authUser?.name || 'Academic Institution';
    });
    ...
    const displayRole = computed(() => {
      return props.authUser?.role || 'Student';
    });
    ...
    const displayAffiliation = computed(() => {
      ...
      return props.authUser?.institution_or_company || 'Academic Institution';
    });
  ...
  template: `
    <div class="space-y-8 max-w-4xl">
      ...
      <template v-if="authUser?.role === 'faculty'">
        ...
        <div class="card p-6 space-y-6">
          <div class="flex flex-wrap items-center justify-between gap-4 border-b border-brand-border pb-4">
            <div>
              <h3 class="font-serif text-lg text-brand-text">MIT OpenCourseWare Modules</h3>
        ...
        <div class="card p-6 space-y-4">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div 
              v-for="asmt in (assessments.length > 0 ? assessments : [
                { id: 'asmt-mit-cs6033', code: 'MIT-CS-6033', title: 'Distributed Computer Systems Engineering', institution: 'MIT EECS Consortium', passing_score: 65, duration_minutes: 45, question_count: 5 },
                { id: 'asmt-stan-ts142', code: 'STAN-CS-142', title: 'Full Stack TypeScript and Modern Reactive Architecture', institution: 'Stanford CS Dept', passing_score: 65, duration_minutes: 45, question_count: 5 },
                { id: 'asmt-bits-ai401', code: 'BITS-AI-401', title: 'Generative AI, Transformer Architecture and LLM Inference', institution: 'BITS Pilani AI Cell', passing_score: 65, duration_minutes: 45, question_count: 5 },
                { id: 'asmt-iitd-sys201', code: 'IITD-SYS-201', title: 'Linux Kernel Internals and High-Throughput Networking', institution: 'IIT Delhi CSE Dept', passing_score: 65, duration_minutes: 40, question_count: 5 }
              ])"
              :key="asmt.id"
            ...
            >
              ...
              <div class="text-xs text-brand-muted flex items-center gap-2">
                ...
                <span>• {{ asmt.question_count || 5 }} questions</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="card p-6">
          <div class="flex items-center justify-between mb-4">
            <div>
              ...
              <p class="text-xs text-brand-muted">Weighted cross-dimensional index spanning Systems, AI/ML, Platform, Data, and Algorithms.</p>
            </div>
          </div>
      ...
      <div v-if="showAddProjectModal" class="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
        <div class="card max-w-lg w-full p-6 space-y-4 shadow-xl">
