...
export default {
  ...
  setup(props, { emit }) {
    ...
    async function loadAnalytics() {
      loading.value = true;
      try {
        const institution = props.facultyUser?.institution_or_company || 'MIT';
    ...
    const filteredCohortStudents = computed(() => {
      ...
      return analytics.value.cohort_students.filter(st => {
      ...
      <div v-else-if="analytics" class="space-y-6">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div class="bg-white rounded-xl border border-brand-border p-5 space-y-1.5 shadow-2xs">
            ...
            <div class="text-[11px] text-emerald-700 font-mono">Verified Competency Baseline</div>
          </div>
