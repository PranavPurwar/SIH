...
export default {
  ...
  setup(_props, { emit }) {
    ...
    const expandedSuiteId = ref(null);
    ...
    function openCreateModal() {
      ...
      form.code = 'MIT-CS-6033';
      form.title = 'Cloud & Distributed Systems Core Assessment';
      form.institution = 'MIT Department of Electrical Engineering & Computer Science';
      form.target_role = 'Distributed Systems & Cloud Platform Engineer';
      form.target_skills = 'Distributed Systems, Go, Kubernetes, gRPC, PostgreSQL';
      form.difficulty = 'Advanced';
      form.duration_minutes = 30;
      form.description = 'Standardized evaluation for backend and distributed systems roles, covering fault tolerance, consensus algorithms, and RPC transport boundaries.';
      form.questions = [
        {
          question_text: 'In the Raft consensus algorithm, what ensures that a leader has all committed entries from previous terms before accepting new client commands?',
          option_a: 'Election restriction: Candidate must contain all committed entries in its log to receive a vote',
          option_b: 'Leader heartbeat verification during randomized election timeouts',
          option_c: 'Two-phase locking across majority quorum before candidate transitions',
          option_d: 'Monotonically decreasing term counters propagated via AppendEntries',
          correct_option: 0,
          explanation: 'Raft guarantees safety via Section 5.4.1 Election Restriction: a voter denies its vote if its own log is more up-to-date than the candidate.'
        }
      ];
    ...
    function openEditModal(suite) {
      ...
      const qList = (suite.questions || []).map(q => ({
    ...
    function onSubmit() {
      const formattedQuestions = form.questions.map((q, idx) => {
        ...
        >
          <div class="flex flex-wrap justify-between items-start gap-3">
            <div>
              ...
              <h2 class="font-serif text-xl text-brand-text font-semibold">{{ decodeHtml(suite.title) }}</h2>
            ...
            <div class="flex items-center gap-2">
              ...
              <button 
                ...
                class="btn-secondary text-xs px-3 py-1 rounded-lg font-medium flex items-center gap-1 hover:border-[#DDD6FE]"
                title="Edit Assessment Suite"
              >
            ...
            <div class="space-y-3 p-4 rounded-xl bg-brand-surface border border-brand-border">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div class="sm:col-span-2">
                  ...
                  <input v-model="form.title" required placeholder="e.g. Cloud and Distributed Systems Core Assessment" class="w-full bg-white border border-brand-border text-brand-text px-3 py-2 rounded-lg text-sm outline-none focus:border-[#581C87]" />
                </div>
                <div>
                  <label class="block text-xs font-medium text-brand-muted mb-1">Code</label>
                  <input v-model="form.code" required placeholder="MIT-CS-6033" class="w-full bg-white border border-brand-border text-brand-text px-3 py-2 rounded-lg text-sm outline-none font-mono uppercase focus:border-[#581C87]" />
                </div>
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input v-model="form.institution" required placeholder="MIT Department of EECS" class="w-full bg-white border border-brand-border text-brand-text px-3 py-2 rounded-lg text-sm outline-none focus:border-[#581C87]" />
                </div>
                <div>
                  <label class="block text-xs font-medium text-brand-muted mb-1">Target Industry Role</label>
                  <input v-model="form.target_role" required placeholder="e.g. Distributed Systems Engineer" class="w-full bg-white border border-brand-border text-brand-text px-3 py-2 rounded-lg text-sm outline-none focus:border-[#581C87]" />
                </div>
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input v-model="form.target_skills" required placeholder="Distributed Systems, Go, Kubernetes, gRPC" class="w-full bg-white border border-brand-border text-brand-text px-3 py-2 rounded-lg text-sm outline-none focus:border-[#581C87]" />
                </div>
