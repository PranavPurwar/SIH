...
export default {
  ...
  setup(props) {
    ...
    const activeTab = ref('browse'); // 'browse' | 'enrolled'
    const enrolledIds = ref(new Set());
    const selectedProgramModal = ref(null);
    ...
    const programTypes = ['All', 'Mentorship', 'Workshop', 'Innovation Challenge', 'Live Project', 'Certification Course'];
    ...
    const enrolledPrograms = computed(() => {
      return programs.value.filter(p => enrolledIds.value.has(p.id));
    });
    ...
    function cancelEnrollment(progId) {
