...
### Evidence Traceability Matrix (`traceability.js`, `TraceabilityMatrix.js`)

Click-to-expand signal audit trail for recruiters and applicant profiles:

- **Ingestion Trace**: Project payload attribution (line ranges), `nomic-embed-text-v2-moe` semantic anchor distance, and extracted keyword signatures.
- **Assessment Trace**: Standardized institutional quiz verification (`IITD-COP-701`, `ANTHROPIC-AI-501`, `MIT-EECS-6033`, etc.) with question-level scoring.
- **Confidence Metric**: Active signal variance calculation ($\sigma = \sqrt{\frac{1}{N}\sum(x_i - \mu)^2}$) measuring cross-signal calibration reliability.
- **Visual Presentation**: Compact, high-signal horizontal matrix table with strict job-specific required skill scoping.

---

### Faculty Development & Opportunities (`faculty.service.ts`, `FacultyOpportunitiesView.js`)

Dedicated portal for academicians to explore and apply for:

- **Faculty Development Programs (FDPs)**: AI/ML architecture, distributed systems, quantum computing.
- **Industrial Sabbaticals / Faculty Internships**: Red Hat, NVIDIA, Cisco, Qualcomm research immersion.
- **Collaborative Research Grants & Consultancies**: Sponsored industry-academia projects with proposal tracking.

---

### Industry Mentorship & Collaboration Hub (`learning.service.ts`, `MentorshipHubView.js`)

Centralized hub connecting students and faculty with industry initiatives:

- **1-on-1 Industry Mentorship**: Deep systems, AI engineering, and architecture mentoring.
- **Technical Bootcamps & Workshops**: GraphRAG, On-Device AI, eBPF telemetry cohorts.
- **Innovation Challenges & Hackathons**: Industry problem statements with prize pools and fast-track hiring.

---

### Institutional Analytics & Placement Dashboard (`analytics.service.ts`, `InstitutionalAnalyticsView.js`)

Cohort-level telemetry and placement intelligence for institutional leadership and TPO teams:

- **Placement Readiness KPI**: Verified student skill baseline across enrolled academic cohorts.
- **Institutional Data Isolation**: Strictly scopes student analytics, funnel telemetry, and cohort directories to the faculty's affiliated institution (`req.user.institution_or_company`). Access to students outside the affiliated university is blocked with HTTP 403 Forbidden.
- **Enrolled Student Cohort Directory & Search**: Real-time filtering by candidate name, degree, or verified technical competencies (e.g., CUDA, Raft, LLVM) with direct portfolio inspection links.
- **In-Demand Skills vs. Curriculum Gap**: Real-time employer requisition demand vs. syllabus depth with remediation advisories.
- **Recruitment Funnel Breakdown**: Application $\to$ Technical Review $\to$ Shortlisted $\to$ Placed transitions.
- **Domain Competency Distribution**: Departmental averages across Systems, AI, Cloud, and Security tracks.

### Faculty Navigation & Academic Tools Popup (`Navbar.js`)

To optimize cognitive load, secondary institutional administration tools are decoupled from the primary horizontal navbar:

- **Primary Horizontal Navigation**: Focuses on core collaboration workflows (`Faculty Opportunities`, `Institutional Analytics`, `Mentorship & Programs`).
- **User Controls Dropdown Popup**: Clicking the faculty member's avatar/name opens a modal popup containing:
  - Faculty identity, academic email, and affiliated institution badge.
  - Institutional tools: `Courseware Modules` (OCW syllabus catalog), `Assessment Suites` (standardized quizzes), and `Institutional Profile` (accreditations & board).
...
```
STUDENTS
  id (PK), name, email (UK), degree, institution
...
FACULTY_PROGRAMS
  id (PK), title, organization, type, description, domain
  stipend_grant, duration, deadline, eligibility

FACULTY_APPLICATIONS
  id (uuid PK), program_id (FK), faculty_name, faculty_email
  institution, proposal_summary, status, applied_at

LEARNING_PROGRAMS
  id (PK), title, company, type, description, target_skills (jsonb)
  duration, mode, stipend_or_perk
...
- **Catalog of Assessment Suites**:
  - **Anthropic Research**: _Frontier Alignment, Constitutional AI & Scalable Evaluation_ (`ANTHROPIC-AI-501`, Advanced) & _Applied LLM Tool-Calling & Agents_ (`ANTHROPIC-AI-201`, Beginner).
  - **Adobe Systems**: _High-Performance Creative Graphics & WebGL_ (`ADOBE-SYS-301`, Intermediate) & _Computational Geometry & Document Engines_ (`ADOBE-IMG-401`, Advanced).
  - **MIT EECS**: _Distributed Systems & Cloud Infrastructure Core Competency_ (`MIT-EECS-6033`, Intermediate).
  - **Stanford CS**: _Full Stack TypeScript & Modern Reactive Architecture_ (`STAN-CS-142`, Intermediate).
  - **IIT Delhi CSE**: _Linux Systems Programming, Kernel Internals & glibc_ (`IITD-COP-701`, Advanced).
  - **BITS Pilani AI**: _Generative AI, Transformer Architecture & LLM Inference_ (`BITS-AI-401`, Advanced).
...
- **Multi-Mode Pipeline Sorting**:
  - _By Posting (Role Title)_: Alphabetical grouping by job title.
  - _Highest Match Score_: Descending match percentage ($100\% \to 0\%$).
  - _Candidate Name (A-Z)_: Alphabetical candidate sort.
  - _Application Status_: Ordered by pipeline phase (`Selected` $\to$ `Shortlisted` $\to$ `Under Review` $\to$ `Applied` $\to$ `Rejected`).
