import { ref, computed, defineComponent } from '../../vue.js';
...
export interface AssessmentViewProps {
  ...
  studentAssessments?: StudentAssessmentAttempt[];
  loading?: boolean;
}
...
export default defineComponent({
  ...
  props: {
    ...
    studentAssessments: {
      type: Array as PropType<StudentAssessmentAttempt[]>,
      default: () => []
    },
    loading: {
      type: Boolean,
  ...
  setup(props, { emit }) {
    ...
    const selectedDifficultyFilter = ref<string>('all');

    // Active Test Runner State
    const activeSuite = ref<AssessmentSuite | null>(null);
    const currentQuestionIndex = ref<number>(0);
    const userAnswers = ref<Record<string, number>>({}); // { question_id: selected_option_index }
    const suiteResult = ref<AssessmentSubmissionResult | null>(null);
    const isSubmitting = ref<boolean>(false);

    function getAttempt(suite: AssessmentSuite | null): StudentAssessmentAttempt | null {
      if (!suite) return null;
      const list = props.student?.assessments || props.studentAssessments || [];
      return list.find((a: StudentAssessmentAttempt) => 
    ...
    const availableRoles = computed(() => {
      ...
      props.assessments.forEach((s: AssessmentSuite) => {
    ...
    const filteredSuites = computed(() => {
      ...
      const matching = props.assessments.filter((s: AssessmentSuite) => {
        const matchesQuery = !q ||
          (s.title || '').toLowerCase().includes(q) ||
          (s.code || '').toLowerCase().includes(q) ||
          (s.institution || '').toLowerCase().includes(q) ||
          (s.target_role || '').toLowerCase().includes(q) ||
          (s.target_skills || []).some((sk: string) => sk.toLowerCase().includes(q));
      ...
      return matching.slice().sort((a: AssessmentSuite, b: AssessmentSuite) => {
    ...
    function startSuite(suite: AssessmentSuite) {
    ...
    const currentQuestion = computed<AssessmentQuestion | null>(() => {
    ...
    async function submitAssessment() {
      ...
      const answersPayload = activeSuite.value.questions.map((q: AssessmentQuestion) => ({
      ...
      try {
        emit('submit-suite-assessment', {
          ...
          callback: (res: AssessmentSubmissionResult) => {
