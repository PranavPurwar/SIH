...
import type { JobApplication, JobListing } from '../../types/index.js';
...
export interface RecruiterCandidatesViewProps {
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    const activeJobFilter = ref<string>(props.selectedJobId || 'all');
    const sortBy = ref<string>('posting');
    const searchFilter = ref<string>('');
    const selectedAppModal = ref<JobApplication | null>(null);

    function openCandidateModal(app: JobApplication) {
    ...
    const availableJobOptions = computed<JobOption[]>(() => {
      const map = new Map<string, JobOption>();

      (props.jobs || []).forEach((j: JobListing) => {
      ...
      (props.candidates || []).forEach((c: JobApplication) => {
    ...
    const selectedJobObj = computed<JobOption | null>(() => {
    ...
    const filteredCandidates = computed<JobApplication[]>(() => {
      ...
      if (searchFilter.value.trim()) {
        ...
        list = list.filter((c: JobApplication) => 
          (c.student?.name || '').toLowerCase().includes(q) ||
          (c.student?.email || '').toLowerCase().includes(q) ||
          (c.job_title || '').toLowerCase().includes(q) ||
          (c.company || '').toLowerCase().includes(q) ||
          (c.status || '').toLowerCase().includes(q) ||
          (c.student?.parsed_skills || []).some((s: string) => s.toLowerCase().includes(q))
        );
      }
      ...
      list.sort((a, b) => {
        ...
        if (sortBy.value === 'status') {
          const statusOrder: Record<string, number> = { 'Selected': 1, 'Shortlisted': 2, 'Under Review': 3, 'Applied': 4, 'Rejected': 5 };
    ...
    function onStatusChange(app: JobApplication) {
      emit('update-status', { id: app.id, status: app.status });
    }

    function viewCandidate(studentId: string) {
